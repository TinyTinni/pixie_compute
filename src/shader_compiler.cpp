...
void reflectBindings(slang::ProgramLayout *layout, std::vector<vk::DescriptorType> &outTypes,
                     std::vector<CompiledShader::Binding> &outBindings)
{
    if (!layout)
        return;
    ...
    auto scan = [&](slang::VariableLayoutReflection *param)
    {
        ...
        outBindings.push_back({param->getName() ? param->getName() : "", bindingSpace, bindingIndex,
                               vkType});
    ...
    for (auto &[idx, type] : bindings)
        outTypes[idx] = type;
    std::sort(outBindings.begin(), outBindings.end(),
                                  ...
                                  const std::string &context)
{
    ...
    std::vector<CompiledShader::Binding> bindings;
    reflectBindings(linked->getLayout(), bindingTypes, bindings);
    ...
    return {std::move(result), pushConstantSize, std::move(bindingTypes), std::move(bindings)};
}
