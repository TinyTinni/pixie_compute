...
GpuImage::GpuImage(GpuContext &ctx, const GpuImageDesc &desc)
    : m_ctx(&ctx), m_format(desc.format), m_width(desc.width), m_height(desc.height),
      ...
      m_imageType(desc.imageType), m_externalImage(false),
      m_layout(vk::ImageLayout::eUndefined)
{
    ...
    vk::ImageCreateInfo imageInfo(
        desc.flags, desc.imageType, desc.format,
        vk::Extent3D(desc.width, desc.height, desc.depth), desc.mipLevels, desc.arrayLayers,
        vk::SampleCountFlagBits::e1, desc.tiling, desc.usage, vk::SharingMode::eExclusive, 0,
...
GpuImage::GpuImage(GpuContext &ctx, uint32_t width, uint32_t height, vk::Format format,
                   vk::ImageUsageFlags usage, vk::ImageType imageType, uint32_t mipLevels,
                   uint32_t arrayLayers, vk::ImageCreateFlags flags, vk::ImageTiling tiling)
    : GpuImage(ctx, GpuImageDesc{width, height, 1, format, usage, imageType, mipLevels,
                                 arrayLayers, flags, tiling})
{
}
...
void GpuImage::upload(const void *data, size_t size, vk::ImageLayout finalLayout,
                      uint32_t mipLevel, uint32_t arrayLayer)
{
    ...
    cmd.begin();
    cmd.imageBarrier(*m_image, vk::ImageLayout::eUndefined,
    ...
    cmd.handle().copyBufferToImage(staging.buffer, *m_image,
...
void GpuImage::download(void *data, size_t size, uint32_t mipLevel, uint32_t arrayLayer)
{
    ...
    vk::BufferImageCopy region(
        ...
        vk::Extent3D(mipExtent(m_width, mipLevel), mipExtent(m_height, mipLevel),
                     m_imageType == vk::ImageType::e3D ? mipExtent(m_depth, mipLevel) : 1));
    cmd.handle().copyImageToBuffer(*m_image, vk::ImageLayout::eTransferSrcOptimal, staging.buffer, 1,
                                   &region);
