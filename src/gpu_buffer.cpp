    ...
    : m_ctx(&ctx), m_size(size), m_type(type)
{
    ...
    vmaGetMemoryProperties(ctx.allocator(), &memProps);
    m_memProps = static_cast<vk::MemoryPropertyFlags>(
        memProps->memoryTypes[resultInfo.memoryType].propertyFlags);

    if (type == Type::HostCoherent)
    {
        if ((m_memProps & vk::MemoryPropertyFlagBits::eHostCoherent) == vk::MemoryPropertyFlags{})
        {
            vmaDestroyBuffer(ctx.allocator(), m_buffer, m_allocation);
            m_buffer = VK_NULL_HANDLE;
            m_allocation = nullptr;
            throw GpuError("HostCoherent buffer memory type is not host-coherent on this device; "
                           "use GpuBuffer::Type::Device with staging transfers instead");
        }
...
void GpuBuffer::swap(GpuBuffer &other) noexcept
{
    ...
    swap(m_memProps, other.m_memProps);
}
...
void GpuBuffer::upload(const void *data, size_t size, size_t offset)
{
    ...
    else
    {
        ...
        // The scoped guard returns staging to the pool even on submission failure.
        GpuCommandBuffer cmd(*m_ctx);
        size_t chunkOffset = 0;
        while (chunkOffset < size)
        {
            size_t chunkSize = std::min(size - chunkOffset, kMaxStagingChunk);
            detail::StagingScoped staging(*m_ctx, m_ctx->acquireStagingBuffer(chunkSize));
            std::memcpy(staging.get().mapped, static_cast<const char *>(data) + chunkOffset,
                        chunkSize);

            cmd.reset();
            cmd.begin();
            cmd.copyBuffer(staging.get().buffer, m_buffer, chunkSize, 0, offset + chunkOffset);
            cmd.end();
            cmd.submitAndWait();
...
void GpuBuffer::download(void *data, size_t size, size_t offset)
{
    ...
    else
    {
        ...
        while (chunkOffset < size)
        {
            ...
            cmd.reset();
            cmd.begin();
            cmd.copyBuffer(m_buffer, staging.get().buffer, chunkSize, offset + chunkOffset, 0);
            cmd.end();
            cmd.submitAndWait();

            std::memcpy(static_cast<char *>(data) + chunkOffset, staging.get().mapped, chunkSize);
            chunkOffset += chunkSize;
        }
    }
}
...
void GpuBuffer::clear(uint32_t value)
{
    ...
    else
    {
        ...
        if (m_size < 4)
            throw GpuError("clear() requires a buffer of at least 4 bytes for a 32-bit fill");
