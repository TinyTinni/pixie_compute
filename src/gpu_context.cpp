...
InstanceResult createInstance(const std::vector<std::string> &extraExtensions,
                              bool enableValidation, uint32_t apiVersion)
{
    ...
    InstanceResult result{vkChecked([&] { return vk::createInstanceUnique(info); },
                                    "vkCreateInstance"),
                          validationEnabled,
                          hasInstanceExtension(VK_EXT_DEBUG_UTILS_EXTENSION_NAME), {}};
...
#endif
    dbgInfo.pfnUserCallback =
        reinterpret_cast<decltype(dbgInfo.pfnUserCallback)>(&debugCallback);
#if defined(__GNUC__) || defined(__clang__)
...
GpuContext::GpuContext(const GpuContextDesc &desc) : m_desc(desc)
{
    ...
    if (m_validation)
        m_debugMessenger = createDebugMessenger(*m_instance, m_desc.debugSeverity, &m_desc);
...
#undef CHECK_FEATURE
    ...
    {
        bool enabledExt =
            std::any_of(enabled.begin(), enabled.end(), [&ext](const char *e)
...
void GpuContext::createCommandPool()
{
    ...
    if (m_hasSeparateTransferQueue && m_transferQueueFamilyIndex != m_queueFamilyIndex)
    {
        ...
        m_transferCommandPool = vkChecked(
            [&] { return m_device->createCommandPoolUnique(transferPoolInfo); },
            "vkCreateCommandPool (transfer)");
    }
    else
    {
        m_transferCommandPool = vkChecked([&] { return m_device->createCommandPoolUnique(poolInfo); },
                                          "vkCreateCommandPool (transfer)");
    }
}
