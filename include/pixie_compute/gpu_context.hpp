...
/// Snapshot captured after a device-loss or other fatal GPU submission error.
struct GpuCrashInfo
{
    bool valid = false;
    ...
    std::string infoDump;
    std::string deviceFault;
    std::vector<std::string> recentLabels;
};
...
struct GpuExtension
{
    ...
    GpuExtension(const char *n, std::function<bool(const char *)> cb)
        : name(n), onMissing(std::move(cb))
    {
    }
...
struct GpuContextDesc
{
    ...
    /// Enable Slang/Vulkan shader printf diagnostics through the validation layer.
    /// Requires VK_KHR_shader_non_semantic_info and validation layers.
    bool enableShaderPrintf = false;
    /// Optional sink for shader printf output. When empty, output uses logSink or stderr.
    std::function<void(const std::string &)> shaderPrintfSink;
    ...
    /// Enable optional VK_EXT_device_fault collection when the driver supports it.
    bool enableDeviceFaultDiagnostics = true;
};
...
class GpuContext
{
    ...
    friend class detail::StagingScoped;
    ...
    bool shaderPrintfEnabled() const noexcept { return m_shaderPrintfEnabled; }
    const GpuCrashInfo &lastCrashInfo() const noexcept { return m_lastCrashInfo; }
    ...
    void reportGpuFailure(vk::Result result, const std::string &operation,
    ...
    bool m_shaderPrintfEnabled = false;
    bool m_deviceFaultEnabled = false;
    GpuCrashInfo m_lastCrashInfo;
