...
[[vk::push_constant]] cbuffer PC : register(b0) { uint4 params; };
...
int main()
{
    try
    {
        ...
        GpuComputePipeline pipeline(ctx, compiled.spirv,
                                    {bufA.descriptorInfo(), bufB.descriptorInfo(),
                                     bufC.descriptorInfo()},
                                    sizeof(pc), cfg.subgroupSize, 1, 1);

        GpuCommandBuffer cmd(ctx);
        cmd.begin();
        cmd.bind(pipeline);
        cmd.pushConstants(pipeline, &pc, sizeof(pc));
        cmd.dispatch(pipeline, kProblemN / cfg.tileN, kProblemM / cfg.tileM, 1);
        cmd.end();
        cmd.submitAndWait();
