...
struct WorkerState
{
    ...
    WorkerState(GpuContext &ctx, uint32_t batchSize)
        : buf_a(ctx, batchSize * sizeof(float), GpuBuffer::Type::Device),
          ...
          pipe(makePipe(ctx, buf_a, buf_b, buf_c))
    {
    }

    private:
    static GpuComputePipeline makePipe(GpuContext &ctx, GpuBuffer &a, GpuBuffer &b, GpuBuffer &c)
    {
        GpuComputePipelineDesc desc;
        desc.slangSource = shader;
        desc.bindings = {a.descriptorInfo(), b.descriptorInfo(), c.descriptorInfo()};
        desc.groupsX = 256;
        return GpuComputePipeline(ctx, desc);
    }
};
